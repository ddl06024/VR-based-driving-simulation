...
public class PedestriansWaypointManagerWindow : EditorWindow
{
    [MenuItem("Tools/Waypoint Editor")]
    public static void Open() 
    {
        //Show existing window instance.
        GetWindow<PedestriansWaypointManagerWindow>();
    }
    ...
    void DrawButtons() 
    {
        ...
        if (GUILayout.Button("Pedestrian Create  Waypoint"))
        {
            Create_pedestrains_Waypoint();

        }

        if (Selection.activeGameObject != null && Selection.activeGameObject.GetComponent<Waypointer>())
        {
            if (GUILayout.Button("Add Branch Waypoint"))
            {
                Create_pedestrains_Branch();
            }           
            if (GUILayout.Button("Create Pedestrains Waypoint Before"))
            {
                Create_pedestrains_waypoint_Before();
            }
            if (GUILayout.Button("Create Pedestrains Waypoint After"))
            {
    ...
    {
        GameObject waypointObject = new GameObject("Waypoint" + waypointRoot.childCount, typeof(Waypointer));
        waypointObject.transform.SetParent(waypointRoot, false);
        waypointObject.gameObject.tag = "Pedestrians";

        Waypointer waypoint = waypointObject.GetComponent<Waypointer>();
        if (waypointRoot.childCount > 1) 
        {
            // 0부터 시작이라 -2를 해준다.
            waypoint.previousWaypoint = waypointRoot.GetChild(waypointRoot.childCount - 2).GetComponent<Waypointer>();
    ...
    void Create_pedestrains_waypoint_Before() 
    {
        ...
        Waypointer newWaypoint = waypointObject.GetComponent<Waypointer>();
        Waypointer selectedWaypoint = Selection.activeGameObject.GetComponent<Waypointer>();
    ...
    void Create_pedestrains_Branch() 
    {
        ...
        Waypointer waypoint = waypointObject.GetComponent<Waypointer>();
        Waypointer branchedFrom = Selection.activeGameObject.GetComponent<Waypointer>();
